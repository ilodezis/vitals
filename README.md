...
<p align="center">
  <strong>Self-hosted health data lake with 69 MCP tools for Claude.ai</strong><br>
...
**📊 Данные и аналитика**
- 13 независимых доменов здоровья
...
- [Домены данных (13 модулей)](#-домены-данных)
...
Все 13 доменов объединены общим интерфейсом `InsightsMixin` (`date`, `domain`, `source` + составной индекс):
...
<details>
<summary><strong>⚗️ 12. Гормональная терапия (ГЗТ / TRT)</strong></summary>

- **Модели**: `HrtCompound`, `HrtDose`, `HrtCycle`, `HrtCycleItem`, `HrtSideEffect`
...
- Полноценные курсы (`HrtCycle`): типы ТРТ / загруз / вечный курс / ПКТ / мост, движок расписания (сегменты + линейная рампа, фиксированная сетка, дробные интервалы вроде E3.5D) и server-side кривая **активного релиза** (сумма экспоненциальных распадов по периоду полураспада × доле активного гормона)
...
<details>
<summary><strong>🔗 13. MCP-интеграция с Claude.ai</strong></summary>

- FastMCP-сервер на `/mcp` (SSE-транспорт)
- OAuth 2.0 + PKCE + Bearer-токены
- 69 инструментов: 30 для чтения + 39 для записи данных (+ 2 ресурса и промпт)
...
**69 инструментов** — Claude может полноценно читать и записывать данные во все домены. Плюс 2 ресурса (`vitals://profile`, `vitals://digest/latest`) и промпт `weekly_review`.
...
| `get_hrt_logs` | Приёмы ГЗТ/TRT, побочные эффекты и активный курс с планом |
| `get_hrt_cycles` | Все курсы ГЗТ с планом по каждому препарату |
...
| `log_hrt_dose` | Запись приёма ГЗТ (мл × концентрация → мг; бренд/лаба/партия; конфликт-гейт) |
| `add_hrt_cycle` | Начать курс ГЗТ (тип; закрывает предыдущий открытый) |
| `add_hrt_cycle_item` | Добавить препарат в курс (расписание: сегменты/рампа или доза + интервал) |
...
- [Supported Domains (13 modules)](#-supported-domains)
- [Technical Architecture](#-technical-architecture)
- [MCP Integration with Claude.ai (69 tools)](#-mcp-integration-with-claudeai)
...
All 13 domains share the `InsightsMixin` interface (`date`, `domain`, `source` + composite index):
...
<details>
<summary><strong>⚗️ 12. Hormone Therapy (HRT / TRT)</strong></summary>

- **Models**: `HrtCompound`, `HrtDose`, `HrtCycle`, `HrtCycleItem`, `HrtSideEffect`
...
- Full cycles (`HrtCycle`): kinds TRT / blast / cruise / PCT / bridge, a schedule engine (segment lists + linear ramp, fixed grid, fractional intervals like E3.5D), and a server-rendered **active-release curve** (sum of exponential decays by half-life × active fraction)
...
<details>
<summary><strong>🔗 13. Claude.ai MCP Integration</strong></summary>

- FastMCP server at `/mcp` (SSE transport)
- OAuth 2.0 + PKCE + Bearer tokens
- 69 tools: 30 read + 39 write (+ 2 resources and a prompt)
