...
async def test_generate_digest_persists_narrative_and_context(db_session):
    ...
    # The system prompt frames it as an analytical navigator.
    assert "навигатор" in llm.prompts[0][0]
...
async def test_assemble_context_includes_intersecting_noise_markers(db_session):
    ...
    assert "период шума" in system_prompt or "периоды шума" in system_prompt
